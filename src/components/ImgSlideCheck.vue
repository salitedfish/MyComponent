...
export default {
  ...
  methods: {
    ...
    verify () {
      ...
      return {
        spliced: Math.abs(left - this.block_x) <= accuracy, //默认为5,判断偏移量是否在精确度之内
        TuringTest: average !== stddev // equal => not person operate，判断是否是人操作
      }
    },
    reset () {
      this.success = false
...
.slide-verify-refresh-icon {
  ...
  height: 34px;
  cursor: pointer;
  /* background: url("./assets/logo.png") 0 -437px; */
...
.slide-verify-slider-mask-item-icon {
  ...
  width: 14px;
  height: 12px;
  /* background: url("./assets/logo.png") 0 -26px; */
  background-size: 34px 471px;
}
